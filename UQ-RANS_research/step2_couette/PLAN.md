# Cross-flow generalization plan (Couette), with pipe and rotating-channel companions

Planning stub written before the cross-flow work, to capture the data formats and the
methodological decisions in the tracked archive. The operative launch brief is the cross-flow CLI
prompt; this file is the durable record. Finding-relevance: the cross-flow
generalization tests transfer of the channel calibrated closure plus calibrated UQ to a
held-out flow type (the supporting result before the separated and compressible
cases).

Status: planning stub. The finding is not yet run. Numbers below are data characterizations
and the channel result, not cross-flow outcomes.

## Goal and finding (the gate)

Calibrate on channel (reuse the channel calibration), predict Couette, and test whether the
calibrated closure plus calibrated UQ transfers to a held-out flow type with reliable
coverage. The success or null criterion is pre-registered. Pre-registered shape
(the pre-registered cross-flow criterion): coverage stays near nominal or degrades gracefully and characterizably
...
a modeled observation uncertainty, never as the DNS statistical `_stdev`. This matches the channel work,
which effectively ran on a 0.5 percent floor. Open item: confirm the relative
level.
...
on the compressible dbns BoundarySpec). Default: a-priori. Form the Couette
...
the channel coefficient posterior rather than recalibrating. A moving-wall 2-D Couette
...
Reuse the channel calibration harness (`ChannelCalibration` and the cross-condition protocol),
